...
def show_home_page(database):
    ...
    left_image, right_image = display_image_uploaders()
    routine = routine_entry_form()
    email = email_entry()
    share_photos = photo_consent()

    if st.button("Get My Assessment"):
        handle_submission(database, email, left_image, right_image, routine, share_photos, modal)
...
def display_header():
    ...
    st.subheader("Get a dermatologist-grade assessment in seconds, powered by MIT research. Upload photos to get started.")
...
def display_image_uploaders():
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Left")
        left_image = st.file_uploader("for the best results take clear photos of your acne.", key="left")
    with col2:
        st.subheader("Right")
        right_image = st.file_uploader("take consistent photos over time, and check back regularly", key="right")
    return left_image, right_image

def email_entry():
    return st.text_input(
        "**Enter your email**: (optional) be the first to know about new features and get personal insights straight in your inbox",
        help="If you provide your email we'll keep track of your progress for you."
    )

def photo_consent():
    share_photos = st.checkbox("Yes, store my photos to help track my progress over time.")
    return share_photos

def handle_submission(database, email, left_image, right_image, routine, share_photos, modal):
    if email and (not is_valid_email(email)):
        st.error("Please enter a valid email address.")
    elif not (left_image or right_image):
        st.error("Please upload at least one image before submitting.")
    else:
        process_submission(database, email, left_image, right_image, routine, share_photos)

    # Modal handling
    if st.session_state.get("show_modal", False):
        display_modal_content(modal)

def process_submission(database, email, left_image, right_image, routine, share_photos):
    model = load_model()
    ...
    # Process images
    image_paths, severity_labels, num_blemishes_list = process_images(left_image, right_image, email, model, database)

    if severity_labels and num_blemishes_list:
        send_results_email(email, severity_labels, num_blemishes_list, image_paths)

def process_images(left_image, right_image, email, model, database):
    image_paths = []
    severity_labels = []
    num_blemishes_list = []
    ...
    return image_paths, severity_labels, num_blemishes_list

def send_results_email(email, severity_labels, num_blemishes_list, image_paths):
    if email:
...
def display_modal_content(modal):
...
def display_andreas_intro():
    st.subheader("Hi, I'm Andrea 👋")
    st.write("""
    After years of struggling with acne, I developed a tool to track and improve my skin. 
    Now, I'm sharing it to help you find what works for you.
    """)
    ...
    st.write("""
    After trying so many of the "solutions" on the market and finding nothing but frustration, 
    I decided to try something new. I focused on experimenting and objectively measuring what worked. 
    
    That's when I discovered a groundbreaking [model](https://arxiv.org/abs/2403.00268) developed by 
    researchers at MIT. This model uses the gold-standard [Hayashi scale](https://pubmed.ncbi.nlm.nih.gov/18477223/), 
    ensuring scientifically accurate assessments.

    By incorporating this technology, we can provide you with reliable, 
    dermatologist-grade acne assessments right from your home. My hope is that by sharing 
    with others, we can collectively find solutions that work.

    I have nothing to sell; my goal is to genuinely help others on their journey to clearer skin. 
    If this tool gains traction and proves helpful, we could potentially crowdsource 
    the cure for acne together. Your feedback and experiences are invaluable in this process.
